...
const Order = () => {
    ...
    const viewOrderDetail = (orderId) => {
        axios.get(`https://localhost:44388/api/OrderDetails?orderId=${orderId}`, {
            headers: {
                Authorization: `Bearer ${getCookie("token")}`
            }
        }).then((response) => {
            setSelectedOrderDetails(response.data.payload);
            setOrderDetailVisible(true); // Hiển thị phần chi tiết đơn hàng
        }).catch((error) => {
